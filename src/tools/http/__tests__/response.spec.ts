...
import { unwrapData, unwrapResult } from "@/tools/http/interceptors/response";
...
describe("响应拦截器", () => {
  describe("响应数据解包", () => {
    ...
    it("解包响应数据,响应体是对象, 并且有data字段直接返回响应体的 result 字段", async () => {
      const { reply, send } = initMockHttp(undefined, unwrapResult);
      reply({
        result: {
          foo: "foo",
        },
    ...
    it("解包响应数据, 响应体是对象, 没有 result 字段, 直接返回响应体", async () => {
      const { reply, send } = initMockHttp(undefined, unwrapResult);
      reply({ foo: "foo" });
